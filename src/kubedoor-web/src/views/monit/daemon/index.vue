      ...
      <div class="edit-container">
        <div class="yaml-editor-container">
          <div class="editor-header">
            <span class="editor-title">DaemonSet YAML配置</span>
            <el-button
              type="primary"
      ...
      <div class="update-method-container">
        ...
        <div class="method-description">
          <p v-if="selectedCreateUpdateMethod === 'create'">
            Create方式用于新建DaemonSet，提交的YAML会被直接创建。
          </p>
...
const handleSearch = async () => {
  ...
  try {
    ...
    searchStore.setNamespace(searchForm.ns || "");
    const res = await getDaemonSetList(
      searchForm.env,
...
const confirmCreateUpdate = async () => {
  ...
  try {
    ...
    if (res.success) {
      ElMessage.success(
        `DaemonSet ${selectedCreateUpdateMethod.value} 更新成功`
      );
      ...
      ElMessage.error(
        res.message || `DaemonSet ${selectedCreateUpdateMethod.value} 更新失败`
      );
    }
  } catch (error) {
    console.error("提交DaemonSet失败:", error);
    ElMessage.error("提交DaemonSet失败");
  } finally {
...
const loadPods = async (row: any) => {
  ...
  try {
    const res = await getDaemonSetPods(row.env, row.namespace, row.name);
    ...
    console.error("获取DaemonSet Pod信息失败:", error);
    ElMessage.error("获取DaemonSet Pod信息失败");
    row.pods = [];
  } finally {
...
const handleRestart = async (row: any) => {
  try {
    ...
    const res = await restartDaemonSet(row.env, row.namespace, row.name);
    if (res.success) {
      ElMessage.success(res.message || "重启任务已触发");
    } else {
      ElMessage.error(res.message || "重启DaemonSet失败");
    }
  } catch (error) {
    if (error !== "cancel" && error !== "close") {
      console.error("重启DaemonSet失败:", error);
      ElMessage.error("重启DaemonSet失败");
    }
  }
};
...
const handleEdit = async (row: any) => {
  ...
  try {
    ...
    console.error("获取DaemonSet内容失败:", error);
    ElMessage.error("获取DaemonSet内容失败");
  }
};
...
const handleSave = async () => {
  if (!yamlEditor || !currentEditResource.value) {
    ElMessage.error("编辑器未初始化或未选择DaemonSet");
    return;
  }
...
const confirmUpdate = async () => {
  ...
  try {
    ...
    if (res.success) {
      ElMessage.success(`DaemonSet ${selectedUpdateMethod.value} 更新成功`);
      updateMethodDialogVisible.value = false;
      editDialogVisible.value = false;
      handleSearch();
    } else {
      ElMessage.error(
        res.message || `DaemonSet ${selectedUpdateMethod.value} 更新失败`
      );
    }
  } catch (error) {
    console.error("更新DaemonSet失败:", error);
    ElMessage.error("更新DaemonSet失败");
  } finally {
...
const handleDelete = async (row: any) => {
  try {
    ...
    if (res.success) {
      ElMessage.success(res.message || "DaemonSet 删除成功");
      handleSearch();
    } else {
      ElMessage.error(res.message || "删除DaemonSet失败");
    }
  } catch (error) {
    if (error !== "cancel" && error !== "close") {
      console.error("删除DaemonSet失败:", error);
      ElMessage.error("删除DaemonSet失败");
    }
  }
};
